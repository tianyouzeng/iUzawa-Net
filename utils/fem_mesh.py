...
def setboundary(elem):
    ...
    edge, E2e, e2E, counts = np.unique(
        allEdge, return_index=True, return_inverse=True, return_counts=True, axis=0
    )
...
def quadpts(order=2):
    ...
    if order == 1:  # Order 1, nQuad 1
        ...
        raise NotImplementedError('Only order 1, 2, 3, 4 are implemented.')
...
class TriMesh2D:
    ...
    def __init__(
        self, node: npt.NDArray | None = None, elem: npt.NDArray | None = None
    ) -> None:
        self.elem = elem
        self.node = node
    ...
    def update_auxstructure(self):
        ...
        numElem = len(elem)  # type: ignore
        numNode = len(node)  # type: ignore

        # every edge's sign
        allEdge = np.r_[elem[:, [1, 2]], elem[:, [2, 0]], elem[:, [0, 1]]]  # type: ignore
        ...
        # edge structures
        self.edge, E2e, e2E, counts = np.unique(
            allEdge, return_index=True, return_inverse=True, return_counts=True, axis=0
        )
        ...
        for i in range(3):
            ...
            self.neighbor[:, i] = np.bincount(
                ixElemLocalEdge[ix, 0], weights=ixElem[ix], minlength=numElem
            )
    ...
    def delete_mesh(self, expr=None):
        ...
        node, elem = self.node, self.elem
        center = node[elem].mean(axis=1)  # type: ignore
        ...
        self.node = node[isValidNode]  # type: ignore
    ...
    def update_gradbasis(self):
        ...
        ve1 = node[elem[:, 2]] - node[elem[:, 1]]  # type: ignore
        ve2 = node[elem[:, 0]] - node[elem[:, 2]]  # type: ignore
        ve3 = node[elem[:, 1]] - node[elem[:, 0]]  # type: ignore
        area = 0.5 * (-ve3[:, 0] * ve2[:, 1] + ve3[:, 1] * ve2[:, 0])
        Dlambda = np.zeros(
            (len(elem), 2, 3)   # type: ignore
        )  # (# elem, 2-dim vector, 3 vertices)   # type: ignore
