...
matrix_s = mat_k_base + mat_m_lump_base / tau + mat_m_lump_base / np.sqrt(alpha)
mg_solver_s.set_V(matrix_s)
pre_s = lambda x: mg_solver_s.V_fun(x[:, None]).flatten()
...
def pre_cond(v: npt.NDArray) -> npt.NDArray:
    '''Preconditioner for the Schur complement system'''
    v1 = mg_solve(v[:num_free_node])
    v2 = mat_m_lump @ v1
    v3 = mgt_solve(v2)
    return v3
...
def mg_solve(v: npt.NDArray) -> npt.NDArray:
    '''Multigrid V-cycle solver for (K + M@Pi/sqrt(alpha))x = v or (K + M@Pi/sqrt(alpha)).T x = v'''
...
def inuzawa(
    ...
    for i in range(max_iter_inuzawa):
        u_prev = u.copy()
        p_prev = p.copy()
        y_prev = y.copy()
        u[free_node] = psi_func(p[free_node], alpha, beta, ua[free_node], ub[free_node])
        res_dual = mat_kt @ p[free_node] + mat_m_lump @ (yd[free_node] - y[free_node])
        y[free_node] = y[free_node] + (0.85 * mat_m_lump_inv) @ res_dual
        res_primal = mat_m_lump @ (u[free_node] + f[free_node]) - mat_k @ y[free_node]
        kr = mg_solve(res_primal)
        kr = mat_m_lump @ kr
        kr = mgt_solve(kr)
        p[free_node] = p[free_node] + 2.0 * kr
        ...
        iter_diff = np.max(
            [
                ...
                norm_m(y[free_node] - y_prev[free_node])
                / np.maximum(norm_m(y_prev[free_node]), 1e-10),
            ]
        )
...
if __name__ == '__main__':
    ...
    for i in tqdm(
        ...
        y, u, p, iter_count, rel_err = inuzawa(
