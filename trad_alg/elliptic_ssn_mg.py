...
import os, inspect
from fem_shared_setup import FEMSetupElliptic
...
read_filename_f_yd = os.path.join(
    root_dir, 'data/elliptic/elliptic_f_yd_test_res256_sz2048.npz'
)
read_filename_ua_ub = os.path.join(
    root_dir, 'data/elliptic/elliptic_ua_ub_test_res256_sz2048.npz'
)
read_filename_u_y = os.path.join(
    root_dir, 'data/elliptic/elliptic_u_y_test_res256_sz2048.npz'
)
...
fem_setup = FEMSetupElliptic(refinement_n, alpha, use_lumped_mass=True)
...
for i in tqdm(range(data_size), total=data_size, bar_format='{l_bar}{bar:36}{r_bar}'):
    ...
    uh_free = np.ones((num_free_node, 1))
    yh_free = np.ones((num_free_node, 1))
    ...
    for i_ssn in range(max_iter_ssn):
        ...
        pre_s_oper = scipy.sparse.linalg.LinearOperator(
...
print(f'Average number of iterations: {np.mean(iter_list):.2f}')
