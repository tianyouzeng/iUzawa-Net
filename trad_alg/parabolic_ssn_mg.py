...
def ssn(
    ...
    for i in range(max_iter_ssn):
        ...
        iter_diff = np.max(
            [
                norm_m(y_step) / np.maximum(norm_m(y[free_node]), 1e-10),
                norm_m(u_step) / np.maximum(norm_m(u[free_node]), 1e-10),
                norm_m(p_step) / np.maximum(norm_m(p[free_node]), 1e-10),
            ]
        )
