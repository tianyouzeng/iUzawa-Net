'''
Generate training data for optimal control of elliptic PDEs (ill-conditioned case).
'''

import numpy as np
import scipy, scipy.sparse, scipy.sparse.linalg
from utils.fem_mg.multigrid import (
    MG,
    rearrange_by_nodes_batched,
    rearrange_by_nodes_inverse,
)
from timeit import default_timer
from tqdm import tqdm
import os, inspect
from trad_alg.fem_shared_setup import FEMSetupEllipticIC
...
is_active_list = np.zeros(data_size, dtype=bool)
...
for i in tqdm(
    ...
    for i_cp in range(max_iter_cp):
        ...
        ph_old = ph_dual
        ###################
        temp1 = scipy.sparse.linalg.cg(mat_k, mat_m @ ph_dual, M=pre_k_oper)[0][:, None]
        uh_free = np.clip((1.0 / (alpha * tau + 1.0)) * (uh_free - tau * temp1), a, b)
        temp2 = scipy.sparse.linalg.cg(
            mat_k, mat_m @ (2.0 * uh_free - uh_old + fh_free), M=pre_k_oper
        )[0][:, None]
        ph_dual = (1.0 / (1.0 + sigma)) * (ph_dual + sigma * temp2 - sigma * ydh_free)
        primal_error_norm = norm_n(uh_old - uh_free) / np.maximum(norm_n(uh_old), 1e-10)
        dual_error_norm = norm_n(ph_old - ph_dual) / np.maximum(norm_n(ph_old), 1e-10)
        error = np.max([primal_error_norm, dual_error_norm])
        if error < tol_rel:
            break
    ...
    delta = 1e-8
    is_strictly_between = (uh >= uah + delta) & (uh <= ubh - delta)
    is_active = ~np.all(is_strictly_between)
    is_active_list[i - data_start] = is_active

    u_list[i - data_start, :, :] = rearrange_by_nodes_inverse(uh, node)
...
num_active = np.sum(is_active_list)
