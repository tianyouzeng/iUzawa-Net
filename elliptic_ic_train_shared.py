...
from models import DUUzawaShared
...
read_filename = os.path.join(
    ...
    'data/elliptic_ic/elliptic_ic_f_yd_train_res64_sz16384.npz',
)
...
read_filename = os.path.join(
    os.path.dirname(os.path.abspath(__file__)),
    'data/elliptic_ic/elliptic_ic_u_y_train_res64_sz16384.npz',
)
with open(read_filename, 'rb') as file:
    data = np.load(file)
...
read_filename = os.path.join(
    os.path.dirname(os.path.abspath(__file__)),
    'data/elliptic_ic/elliptic_ic_ua_ub_train_res64_sz16384.npz',
)
with open(read_filename, 'rb') as file:
    data = np.load(file)
...
for ep in range(epochs):
    ...
    for u, f, yd, y, u_a, u_b in train_loader:
        optimizer.zero_grad()
...
torch.save(model.state_dict(), 'elliptic_ic_du_uzawa_shared_layer_6.pt')
