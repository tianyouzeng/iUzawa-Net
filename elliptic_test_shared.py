...
read_filename = os.path.join(
    ...
    'data/elliptic/elliptic_f_yd_test_res64_sz2048.npz',
)
...
read_filename = os.path.join(
    os.path.dirname(os.path.abspath(__file__)),
    'data/elliptic/elliptic_u_y_test_res64_sz2048.npz',
)
with open(read_filename, 'rb') as file:
    data = np.load(file)
...
read_filename = os.path.join(
    os.path.dirname(os.path.abspath(__file__)),
    'data/elliptic/elliptic_ua_ub_test_res64_sz2048.npz',
)
with open(read_filename, 'rb') as file:
    data = np.load(file)
...
model.load_state_dict(torch.load('trained_models/elliptic_du_uzawa_shared_layer_10.pt'))
model.to(device)
model.eval()
